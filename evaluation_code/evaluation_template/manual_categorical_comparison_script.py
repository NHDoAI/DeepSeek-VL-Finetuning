...
def main():
    ...
    model_results_file = ["./extraction_results/real/extraction_results_model_real_20250531_185309.csv", "./extraction_results/simulation/extraction_results_model_simulation_20250531_185309.csv"] # IMPORTANT: Update this path
    ground_truth_file = ["./extraction_results/real/ground_truth_real.csv", "./extraction_results/simulation/ground_truth_simulation.csv"]       # IMPORTANT: Update this path
    ...
    data_type_for_run = ["real", "simulation"] # IMPORTANT: Update as needed
    
    # Base directory where 'data_type_for_run' subfolder will be created for reports
    output_directory = ["./manual_comparison_reports/", "./manual_comparison_reports/"]                 # IMPORTANT: Update as needed

    # --- End Configuration ---

    for i in range(len(model_results_file)):
        if not os.path.exists(model_results_file[i]) or not os.path.exists(ground_truth_file[i]):
            print(f"\nError: One or both input CSV files do not exist. Please check the paths:")
            if not os.path.exists(model_results_file[i]):
                print(f"  Model results file not found: {os.path.abspath(model_results_file[i])}")
        ...
        stats_file, counts_file = perform_comparison_and_generate_reports(
            model_results_csv_path=model_results_file[i],
            ground_truth_csv_path=ground_truth_file[i],
            data_type=data_type_for_run[i],
            output_base_dir=output_directory[i],
            timestamp=current_timestamp
        )

        if stats_file and counts_file:
            print(f"\nComparison process complete for data type '{data_type_for_run[i]}'.")
            print(f"  Stats report: {os.path.abspath(stats_file)}")
            print(f"  Counts report: {os.path.abspath(counts_file)}")
        else:
            print(f"\nComparison process for '{data_type_for_run[i]}' encountered errors or produced no output.")
