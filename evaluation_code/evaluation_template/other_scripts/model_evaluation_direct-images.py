...
"""get the model answers directly from images. Find images names in test_images_dir then join the image name with test_images_dir and generate answer for that image, then save the results to a folder."""
#NOTE 23-05-25: THIS CURRENTLY DOES NOT WORK BECAUSE NOT ONLY HAS IT NOT BEEN ADAPTED TO NEW TEST SET, NEW FINE-TUNED MODEL WITH EXTRA TOKENS, BUT ALSO THE FIXED USER PROMPT DOES NOT TAKE INTO ACCOUNT REAL VS SIMULATED DATA. SUGGESTION: MAKE A COPY OF THE model_evaluation_test-set.py AND ADAPT IT TO HAVE TWO FIXED PROMPTS CASES, ONE FOR REAL AND ONE FOR SIM SET. OR JUST USE THE model_evaluation_test-set.py SCRIPT DIRECTLY. THIS IS ONLY FOR BACKUP PURPOSES.
...
def main():
    ...
    model_path = "/home/ubuntu/DeepSeek-VL-Finetuning/training_code/cluster_training/experimental_1.3b_less-lora/models/merged_model_later_save/"
...
{decision}"""
    ...
    test_files = [os.path.join(test_images_dir, fname) for fname in os.listdir(test_images_dir) 
