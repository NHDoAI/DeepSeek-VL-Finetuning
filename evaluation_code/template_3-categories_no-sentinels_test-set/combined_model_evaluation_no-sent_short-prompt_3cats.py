"""
For evaluation of data with shortened prompts that only predict 3 categories.
...
def extract_phrase(text_target, phrase_query, score_cutoff=0):
    """Extract a phrase from target text using fuzzy matching. The phrase should contain the categories info and will be used for that."""
    ...
    for i in range(target_word_count + 1 - query_word_count):
        """takes a target text and a query phrase. It breaks both into words and searches for a sequence of words in the target that closely matches the query phrase. It uses rapidfuzz.fuzz.ratio to score the similarity."""
...
def detect_category(text_target, categories_query, score_cutoff=75):
    """Detect category from the extracted text using fuzzy matching."""
    best_score = 0
...
def extract_text_from_json(json_file):
    """Extract the whole text content from JSON file."""
...
def merge_model(base_model_name, adapter_checkpoint, output_dir):
    ...
    fp16_model_path = os.path.join(output_dir, "fp16_model") # path to save the non-quantized model (in fp16 precision)
...
def quantize_model(model_path, output_dir):
    """Quantize the model to 4-bit. Since the merged model is in fp16 precision, we need to quantize it to 4-bit for faster inference."""
    print(f"Quantizing model from {model_path}...")
    
    # Prepare quantization config

    vl_chat_processor = VLChatProcessor.from_pretrained(model_path, trust_remote_code=True) # load the processor from the model path
    ...
    model_4bit = AutoModelForCausalLM.from_pretrained( # load the quantized 4-bit model with the quantization config
        model_path,            # the fp16 weights you just saved
        quantization_config=quantization_config,
        device_map="auto",          # loads sharded across available GPUs
    )

    merged_4bits_path = os.path.join(output_dir, "4bit_model") # path to save the quantized 4-bit model
    model_4bit.save_pretrained(merged_4bits_path, safe_serialization=True) # save the quantized 4-bit model
...
def run_inference(model_path, base_labels_dir, mode = "fp16"): # default mode is fp16 but should be 4-bit when actually loaded for faster inference
    ...
    main_output_dir = os.path.join("./eval_output", f"inference_{timestamp}") # path to save the inference results
    ...
    if mode == "fp16":
        ...
        quantization_config = BitsAndBytesConfig( # passing quantization config to the model again as safety measure, if the model was not quantized before, it will be quantized now, if it was already quantized, no new quantization will be triggered.
            load_in_4bit=True,
            bnb_4bit_compute_dtype=torch.float16,
            bnb_4bit_use_double_quant=True,
        )
    ...
    vl_gpt = vl_gpt.eval() # set the model to evaluation mode
    
    sub_dirs_to_process = ["real/labels/", "simulation/labels/"] # list of subdirectories with the data to be processed

    # loop through the json label files of each subdirectory (real and simulation) and perform inference for each of them
    for sub_dir_name in sub_dirs_to_process:
        ...
        # loop through the individual json label files
        for i, label_file_path in enumerate(json_label_files):
            ...
            try:
                ...
                # extract the necessary content from the json file
                img_path_from_json = data["conversation"][0]["images"][0] # path to the image
                current_user_prompt = data["conversation"][0]["content"] # the user prompt
            ...
            # object that contains all information to be passed to the model
            conversation = [
                {
                    "role": "User",
            ...
            try:
                pil_images = load_pil_images(conversation) # method to preprocess the input image
            except Exception as e:
                print(f"    Error loading image {img_path_from_json} specified in {input_json_filename}: {e}. Skipping.")
                continue
            
            prepare_inputs = vl_chat_processor( # method to preprocess all input data
                conversations=conversation, 
                images=pil_images, 
                force_batchify=True
            ).to(vl_gpt.device)
            prepare_inputs.pixel_values = prepare_inputs.pixel_values.to(torch.float16) # convert the pixel values to float16 since model needs them to be in this precision
...
def extract_categories_from_results(results_dir, output_dir, overall_timestamp):
    ...
    for data_type_info in data_types_to_process:
        ...
        if not json_label_files:
            ...
            with open(empty_output_file, 'w', newline='') as f: # create an empty csv file to store the extracted categories
        ...
        # loop for extracting categories from the model responses
        for data_item in data_text_current_type:
            ...
            for option in lane_options:
                ...
                # find phrase that match 'Lane:' + 'one of the lane_options'
                temp_matched_phrase, temp_score = extract_phrase(data_item.lower(), current_dynamic_query_lane.lower())

                if temp_score > best_score_for_lane_extraction: # check the score of the current phrase vs the best score so far. If it is better, update the best score and the best matched phrase.
                    best_score_for_lane_extraction = temp_score
                    best_matched_phrase_for_lane = temp_matched_phrase

            # detect and store the category for the lane
            ...
            for option in obstacles_options:
                ...
                # find phrase that match 'Obstacles:' + 'one of the obstacles_options'
                temp_matched_phrase, temp_score = extract_phrase(data_item.lower(), current_dynamic_query_obstacle.lower())

                if temp_score > best_score_for_obstacle_extraction: # check the score of the current phrase vs the best score so far. If it is better, update the best score and the best matched phrase.
            ...
            for option in decision_options:
                ...
                # find phrase that match 'Decision:' + 'one of the decision_options'
                temp_matched_phrase, temp_score = extract_phrase(data_item.lower(), current_dynamic_query_decision.lower())

                if temp_score > best_score_for_decision_extraction: # check the score of the current phrase vs the best score so far. If it is better, update the best score and the best matched phrase.
        ...
        for i in range(len(file_names_current_type)):
            # Create a dictionary to store the extracted results for the current file
            result = {
            ...
            results_current_type.append(result) # append the extracted results to the list of results
...
def analyze_and_generate_report(model_results_csv_paths_list, overall_timestamp,
    ...
    for model_results_csv in model_results_csv_paths_list:
        try:
            ...
            if data_type not in ["real", "simulation"]: # check if the data type (real or simulation) is valid
                print(f"  Warning: Could not determine data type for {model_results_csv} from dir '{data_type}'. Skipping.")
                continue

            ground_truth_csv = os.path.join(model_file_parent_dir, f"ground_truth_{data_type}.csv") # get the path to the ground truth csv file

            if not os.path.exists(ground_truth_csv): # check if the ground truth csv file exists
            ...
            gt_df = pd.read_csv(ground_truth_csv) # read the ground truth csv file
            pred_df = pd.read_csv(model_results_csv) # read the model results csv file
            merged_df = pd.merge(gt_df, pred_df, on='file_name', suffixes=('_gt', '_pred')) # merge the ground truth and model results csv files on the file_name column

            categories = [col for col in gt_df.columns if col != 'file_name'] # get the categories from the ground truth csv file
            
            report_buffer = io.StringIO() # create a buffer to store the report
            category_scores = {} # create a dictionary to store the scores for each category

            # loop for calculating the metrics for each category
            for category in categories:
                ...
                y_true = merged_df[gt_col] # get the ground truth values for the current category
                y_pred = merged_df[pred_col] # get the predicted values for the current category
                
                if category == 'obstacle':
                    labels = obstacle_order # if the category is obstacle, use the obstacle_order
                else:
                    labels = sorted(list(set(y_true.unique()) | set(y_pred.unique()))) # if the category is not obstacle, use the unique values from the ground truth and predicted values

                cm = confusion_matrix(y_true, y_pred, labels=labels) # generate the confusion matrix. Columns are prediction, rows are ground truth
                cm_df = pd.DataFrame(cm, index=labels, columns=labels) # convert the confusion matrix to a dataframe

                # write the confusion matrix to the report
                ...
                metrics_df = calculate_precision_recall(cm, labels) # calculate the precision, recall, and specificity for each sub-category
                report_buffer.write("Metrics per Sub-category:\n")
                report_buffer.write(metrics_df.to_string(index=False))
                report_buffer.write("\n\n")

                # Calculate the custom metrics for each category

                """Use "beta" values for calculating the F-beta score for each category.
                beta = 1.0 (F1-score) gives equal weight to precision and recall.
                beta > 1.0 (like 2.0 for decision) gives more weight to recall, meaning it's more important to identify all positive cases, even at the cost of some false positives. This is crucial for safety-critical "decisions".
                beta < 1.0 gives more weight to precision."""
            ...
            # Complete Exact Accuracy (Percentage of predictions that are correct for all categories)
            ...
            # Get the individual category scores and calculate the composite score
...
def main():
    ...
    with open(log_file_name, 'w') as log_file:
        ...
        try:
            ...
            # --- User Configuration ---
            base_model_name = os.getenv('MODEL_1.3B_CHKPOINT') # get the path to the base model
            adapter_checkpoint = "/home/ubuntu/DeepSeek-VL-Finetuning/training_stage/new_cluster_training_runs_final-ver_models/250625_full_b12-s123_dropout-0.05_loss/best_chkpoint/adapter/" # get the path to the adapter checkpoint
            merged_model_dir = "/home/ubuntu/DeepSeek-VL-Finetuning/training_stage/new_cluster_training_runs_final-ver_models/250625_full_b12-s123_dropout-0.05_loss/best_chkpoint/merged_model/" # set the path to save the merged model
            test_labels_dir = "./test/" # get the path to the labels of the set we want to evaluate
            extraction_dir = "./extraction_results/" # set the path to save the extracted categories results
            
            # flags to control the flow of the pipeline
            merge_model_flag = True # whether to merge the model with the adapter
            quantize_model_flag = True # whether to quantize the model
            run_inference_flag = True # whether to run inference on the images
            extract_cat_flag = True # whether to extract categories from the results
            ...
            WEIGHT_DECISION = 0.5 # more weight to decision
            WEIGHT_LANE = 0.3
            WEIGHT_OBSTACLE = 0.2
