...
def run_inference_not_merge(model_path, adapter_checkpoint, images_dir):
    """Run inference on images and save results."""
    print(f"Starting inference process...")
    print(f"Model path: {model_path}")
    print(f"Adapter checkpoint: {adapter_checkpoint}")
    ...
    # Load model with adapter
    vl_gpt = PeftModel.from_pretrained(vl_gpt, adapter_checkpoint, is_trainable=False)
    vl_gpt = vl_gpt.eval()
...
def compare_results_with_ground_truth(model_results_csv, ground_truth_csv, output_dir):
    ...
    with open(category_counts_file, 'w', newline='') as f:
        ...
        for category, counts in category_counts.items():
            for count_type, count_dict in counts.items():
                for value, count in count_dict.items():
                    writer.writerow([category, count_type, value, count])
...
def main():
    ...
    adapter_checkpoint = "/home/ubuntu/DeepSeek-VL-Finetuning/training_code/cluster_training/after_first-epoch_fix_models/1.3b_less-lora_rerun/best-epoch/adapter/"
    merged_model_dir = "/home/ubuntu/DeepSeek-VL-Finetuning/training_code/cluster_training/after_first-epoch_fix_models/1.3b_less-lora_rerun/best-epoch/merged_model_best-eval"
    ...
    inference_results_dir = run_inference_not_merge(base_model_name, adapter_checkpoint, test_images_dir)
