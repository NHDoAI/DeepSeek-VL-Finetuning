...
def run_inference(model_path, base_labels_dir, mode="fp16", evaluation_mode="real"):
    """Run inference using prompts and image paths from JSON files."""
    print(f"Starting inference process for mode: {evaluation_mode}...")
    print(f"Model path: {model_path}")
    print(f"Base labels directory: {base_labels_dir}")
    
    # Create main output directory with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    main_output_dir = os.path.join("./eval_output", f"inference_{evaluation_mode}_{timestamp}")
    ...
    sub_dir_name = os.path.join(evaluation_mode, "labels")
    ...
    print(f"Found {len(json_label_files)} JSON files to process in {evaluation_mode} directory")
    
    for i, label_file_path in enumerate(json_label_files):
        input_json_filename = os.path.basename(label_file_path)
        print(f"  Processing JSON file {i+1}/{len(json_label_files)} ({evaluation_mode}): {input_json_filename}")
        ...
        # --- DEBUG (Optional but helpful for one more run) ---
        print(f"    DEBUG: Number of generated tokens: {len(generated_tokens)}")
        print(f"    DEBUG: Decoded Answer: '{answer.strip()}'")
...
def extract_categories_from_results(results_dir, output_dir, overall_timestamp, mode="real"):
...
def analyze_and_generate_report(model_results_csv_paths_list, overall_timestamp,
    ...
    script_base_name = f"{os.path.basename(os.path.dirname(os.path.realpath(__file__)))}_mode-{mode}"
    ...
    for model_results_csv in model_results_csv_paths_list:
        try:
            ...
            output_filename = f"{script_base_name}_report.txt"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run model evaluation pipeline for 'real' or 'simulation' data.")
    parser.add_argument('--mode', type=str, required=True, choices=['real', 'simulation'], 
                        help="The evaluation mode: 'real' or 'simulation'.")
    args = parser.parse_args()
    mode = args.mode

    # --- Setup logging to file ---
    log_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file_name = f"evaluation_pipeline_log_{mode}_{log_timestamp}.log"
    ...
    with open(log_file_name, 'w') as log_file:
        ...
        try:
            ...
            if mode == 'simulation':
                adapter_checkpoint = "/home/ubuntu/DeepSeek-VL-Finetuning/training_stage/new_cluster_training_runs_final-ver_models/Real-training_b6-s322_from-sim-b6-s23-full-epoch/manual_eval_chkpoint_12/adapter/"
                merged_model_dir = "/home/ubuntu/DeepSeek-VL-Finetuning/training_stage/new_cluster_training_runs_final-ver_models/Real-training_b6-s322_from-sim-b6-s23-full-epoch/manual_eval_chkpoint_12/merged_model_sim/"
            else: # mode == 'real'
                # PLEASE UPDATE these paths for your 'real' model's checkpoints and desired output directory
                adapter_checkpoint = "/home/ubuntu/DeepSeek-VL-Finetuning/training_stage/new_cluster_training_runs_final-ver_models/Real-training_b6-s322_from-sim-b6-s23-full-epoch/manual_eval_chkpoint_12/adapter/"
                merged_model_dir = "/home/ubuntu/DeepSeek-VL-Finetuning/training_stage/new_cluster_training_runs_final-ver_models/Real-training_b6-s322_from-sim-b6-s23-full-epoch/manual_eval_chkpoint_12/merged_model_real/"
            ...
            if run_inference_flag:
                ...
                inference_results_dir = run_inference(merged_model_path, test_labels_dir, mode="4bit", evaluation_mode=mode)

            else:
                # Example path if you skip inference, update as needed
                inference_results_dir = f"./eval_output/inference_{mode}_20250525_000824"
            ...
            if extract_cat_flag:
                ...
                model_results_csv = extract_categories_from_results(inference_results_dir, extraction_dir, overall_timestamp, mode=mode)
            else:
                # Example paths if you skip extraction, update as needed
                overall_timestamp = "20250525_010633" # Example timestamp
                model_results_csv = [f"./extraction_results/{mode}/{os.path.basename(os.path.dirname(os.path.realpath(__file__)))}'_{mode}.csv"]
