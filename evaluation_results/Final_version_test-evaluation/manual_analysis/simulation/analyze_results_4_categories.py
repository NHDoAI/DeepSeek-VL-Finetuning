...
def analyze_predictions(result_filepath, ground_truth_filepath, 
                        beta_decision, beta_lane, beta_obstacle, beta_final_decision,
                        weight_decision, weight_lane, weight_obstacle, weight_final_decision,
                        obstacle_order):
    """
    ...
    try:
        ...
        for category in categories:
            ...
            elif category == 'final_decision':
                f_beta = fbeta_score(y_true, y_pred, beta=beta_final_decision, average='macro', labels=labels, zero_division=0)
                category_scores['final_decision_fbeta'] = f_beta
                report_buffer.write(f"Macro F-{beta_final_decision} Score: {f_beta:.4f}\n")
        ...
        final_decision_fbeta = category_scores.get('final_decision_fbeta', 0)

        composite_score = (
            decision_fbeta * weight_decision +
            lane_fbeta * weight_lane +
            obstacle_kappa * weight_obstacle +
            final_decision_fbeta * weight_final_decision
        )
        ...
        report_buffer.write(f"Final Decision Macro F-{beta_final_decision} Score: {final_decision_fbeta:.4f}\n\n")
        
        report_buffer.write(f"Composite Score: {composite_score:.4f}\n")
        report_buffer.write(f"(Weights: Decision F-beta={weight_decision}, Lane F-beta={weight_lane}, Obstacle Kappa={weight_obstacle}, Final Decision F-beta={weight_final_decision})\n")
...
def main():
    ...
    """
    # --- User Configuration ---
    # This ground truth file should contain the 4 categories.
    GROUND_TRUTH_FILE = './ground_truth_real_4cats.csv'
    
    # IMPORTANT: Update these file paths to your 4-category prediction CSVs.
    RESULT_FILES = [
        './path_to_your_4_category_prediction_1.csv',
        './path_to_your_4_category_prediction_2.csv',
    ...
    BETA_FINAL_DECISION = 2.0   # Emphasize recall for Final Decision

    # Composite score weights (ensure they sum to 1.0)
    WEIGHT_DECISION = 0.25
    WEIGHT_LANE = 0.3
    WEIGHT_OBSTACLE = 0.2
    WEIGHT_FINAL_DECISION = 0.25
