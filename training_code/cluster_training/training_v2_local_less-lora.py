...
run = wandb.init(
    ...
    name="1.3b_lower-rank_less-lora_cluster_batch6",            # optional: custom run name
    mode="offline"
)
...
checkpoint_path = "./saved_model_1.3b_lower-rank_less-lora_experiment"
...
if early_stop_flag:
    early_stopper = EarlyStopping(patience=3, delta=0, checkpoint_path=checkpoint_path)
...
for epoch in range(num_epochs): # epochs loop
    ...
    if current_epoch == 1:
        model.save_pretrained("./less-lora_first_epoch_chkpoint")
        ...
        with open(os.path.join("./less-lora_first_epoch_chkpoint", "checkpoint_metadata.json"), "w") as f:
            json.dump(metadata, f, indent=4)
