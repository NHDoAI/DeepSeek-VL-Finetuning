...
hyperparameters = {
    ...
    "run_name_prefix": "1.3b_Option-B/third_version/no-sentinels-run", # Prefix for run names and checkpoint subdirs
    ...
    "early_stopping_active_ranges": [[2550, 20000]], # New: List of [start_step, end_step] ranges for active early stopping. Example: [[0, 2000], [4000, 6000]]
    ...
    "manual_checkpoint_steps":[2199, 2299],
...
def evaluate_model(model, data_dataloader):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_dataloader):
            ...
            valid_mask_acc = (shift_labels != -100)
            num_correct_tokens += (preds_acc[valid_mask_acc] == shift_labels[valid_mask_acc]).sum().item()
            num_total_tokens += valid_mask_acc.sum().item()
    ...
    #             for sent_tok in sentinel_ids}
    overall_acc = num_correct_tokens / num_total_tokens if num_total_tokens > 0 else 0.0
    ...
    for batch_idx, batch in enumerate(train_dataloader): # batches loop
        ...
        if not applied_segment_details["is_warmup"] and \
           steps_in_current_phase_for_eval_counter % hyperparameters["eval_every_n_steps"] == 0:
            ...
            test_loss_real, overall_test_acc_real = evaluate_model(model, test_dataloader_real)
            ...
            test_loss_sim, overall_test_acc_sim = evaluate_model(model, test_dataloader_sim)
            ...
            last_eval_loss_real, last_overall_eval_acc_real, last_eval_loss_sim, last_overall_eval_acc_sim, last_avg_eval_loss, last_avg_eval_acc, last_test_loss_real, last_overall_test_acc_real, last_test_loss_sim, last_overall_test_acc_sim, last_avg_test_loss, last_avg_test_acc = eval_loss_real, overall_eval_acc_real, eval_loss_sim, overall_eval_acc_sim, avg_eval_loss, avg_eval_acc, test_loss_real, overall_test_acc_real, test_loss_sim, overall_test_acc_sim, avg_test_loss, avg_test_acc
