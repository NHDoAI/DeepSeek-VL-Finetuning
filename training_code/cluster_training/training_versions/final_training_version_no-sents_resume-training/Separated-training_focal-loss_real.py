...
# ------ Configuration ------
# MASTER SWITCH FOR TRAINING STAGE
# 'simulation': Train a new adapter from the base model on simulation data.
# 'real_finetune': Load a pre-trained adapter and continue training on real data.
training_stage = "real_finetune" # Can be 'simulation' or 'real_finetune'
...
hyperparameters = {
    "common": {
        ...
        "wandb_project_name": "deepseek-vl-training_final-train_short-prompts-lidar",
        # "real_image_eval_weight": 0.8,
        # "sim_image_eval_weight": 0.2,

        "focal_loss_gamma": 2.0, # gamma parameter for focal loss
        "focal_loss_alpha": 2.0, # alpha parameter for focal loss, can be a float or a list of weights
    ...
    "simulation": {
        "run_name_prefix": "1.3b_sim-train_stage1",
        "adapter_to_load": None, # Train a new adapter from scratch
        "train_data_env_var": "TRAIN_LABELS_PATH_SIM",
        "eval_data_env_var": "EVAL_LABELS_PATH_SIM",
        "test_data_env_var": "TEST_LABELS_PATH_SIM",
        "max_epochs": 15,
        "learning_rate_phases": [2e-4, 5e-5, 5e-5],
        "weight_decay_phases": [0.0, 0.0, 0.0],
        "grad_clip_norm_phases": [1.0, 1.0, 1.0],
    },
    "real_finetune": {
        "run_name_prefix": "1.3b_real-finetune_stage2",
        # IMPORTANT: This path must be updated to point to the desired adapter from stage 1
        "adapter_to_load": "./checkpoints/1.3b_sim-train_stage1_loss/best_chkpoint", 
        "train_data_env_var": "TRAIN_LABELS_PATH_REAL",
        "eval_data_env_var": "EVAL_LABELS_PATH_REAL",
        "test_data_env_var": "TEST_LABELS_PATH_REAL",
        "max_epochs": 10,
        "learning_rate_phases": [5e-5, 1e-5, 5e-6], # Lower LR for fine-tuning
        "weight_decay_phases": [0.0, 0.0, 0.0],
        "grad_clip_norm_phases": [1.0, 1.0, 1.0],
    }
}
...
# Start with common hyperparameters
active_config = hyperparameters["common"].copy()
# Update with stage-specific hyperparameters
stage_specific_config = hyperparameters[training_stage]
active_config.update(stage_specific_config)


# Initialize with Phase 1 values for optimizer and initial clipping
# LR will be min_lr if WU1 has steps, otherwise P1's LR.
active_config["learning_rate"] = active_config["min_lr"] if active_config["warmup_steps_phases"][0] > 0 else active_config["learning_rate_phases"][0]
active_config["weight_decay"] = active_config["weight_decay_phases"][0]
active_config["grad_clip_norm"] = active_config["grad_clip_norm_phases"][0]
# Initialize phase-dependent token weights and eval frequency (for WU1/P1)
#active_config["keyword_weight"] = active_config["keyword_weight_phases"][0]
#active_config["background_weight"] = active_config["background_weight_phases"][0]
active_config["eval_every_n_steps"] = active_config["eval_every_n_steps_phases"][0]


# Construct wandb_run_name, checkpoint_dir and best_checkpoint_path using other hyperparameters
active_config["wandb_run_name"] = f"{active_config['run_name_prefix']}_{eval_mode}"
active_config["checkpoint_dir"] = os.path.join(
    active_config["base_checkpoint_dir"],
    f"{active_config['run_name_prefix']}_{eval_mode}/"
)
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_dataloader):
            ...
            loss_fct_none = FocalLoss(
                gamma=active_config["focal_loss_gamma"], 
                alpha=active_config["focal_loss_alpha"], 
                reduction='none', 
                ignore_index=-100
            )
...
MASTER_SEED = active_config["master_seed"]          # change this between experiments
...
segment_start_steps = {}
conceptual_pb = active_config["phase_boundaries"]
warmup_steps = active_config["warmup_steps_phases"]
...
# --- Initialize wandb ---
run = wandb.init(
    project=active_config["wandb_project_name"],  # name of your project in wandb
    name=active_config["wandb_run_name"], # Constructing a more dynamic run name
    mode="online"
)
...
# background_weight  = active_config["background_weight"]                                            
...
lora_rank = active_config["lora_rank"]
...
if training_stage == "simulation":
    print("--- Training Stage: SIMULATION ---")
    print("Initializing new LoRA adapters for the base model.")
    ...
    print("--- Training Stage: REAL FINE-TUNE ---")
    adapter_path = active_config.get("adapter_to_load")
    if not adapter_path:
        print("ERROR: For 'real_finetune' stage, 'adapter_to_load' must be specified in hyperparameters.")
        sys.exit(1) # Exit with an error code

    print(f"Loading pre-trained LoRA adapter from: {adapter_path}")
    model = PeftModel.from_pretrained(model, adapter_path, adapter_name="adapter", is_trainable=True)
    print("Pre-trained LoRA adapter loaded and set to trainable.")
...
best_checkpoint_path = os.path.join(active_config["checkpoint_dir"], "best_chkpoint")

# --- Prepare Training Loop ---

# --- Prepare Datasets ---

train_labels_dir_env_var = active_config["train_data_env_var"]
train_labels_dir = os.getenv(train_labels_dir_env_var)
print(f"Loading training data from ENV VAR '{train_labels_dir_env_var}': {train_labels_dir}")

eval_labels_dir_env_var = active_config["eval_data_env_var"]
eval_labels_dir = os.getenv(eval_labels_dir_env_var)
print(f"Loading evaluation data from ENV VAR '{eval_labels_dir_env_var}': {eval_labels_dir}")

test_labels_dir_env_var = active_config["test_data_env_var"]
test_labels_dir = os.getenv(test_labels_dir_env_var)
print(f"Loading test data from ENV VAR '{test_labels_dir_env_var}': {test_labels_dir}")
...
train_dataloader = torch.utils.data.DataLoader(
    ...
    num_workers=active_config["num_workers_dataloader"],
...
max_epochs = active_config["max_epochs"]

# --- Scheduler Hyperparameters ---
lr_scheduler_patience = active_config["lr_scheduler_patience"]
lr_scheduler_factor = active_config["lr_scheduler_factor"]
min_lr = active_config["min_lr"]
#max_lr_reductions = active_config["max_lr_reductions"]
scheduler_mode = active_config["lr_scheduler_mode"]
...
initial_optimizer_lr = active_config["min_lr"] if active_config["warmup_steps_phases"][0] > 0 else active_config["learning_rate_phases"][0]
optimizer = bnb.optim.AdamW8bit(
    model.parameters(),
    lr=initial_optimizer_lr, # Set initial LR
    betas=(active_config["beta1"], active_config["beta2"]),
    weight_decay=active_config["weight_decay_phases"][0], # Initial WD (WU1/P1)
    is_paged = True
) # paged optimizer
...
wandb.config.update(
    {
        ...
        "training_stage": training_stage, # Log the current training stage
        "run_name": active_config["wandb_run_name"],
        "batch_size": active_config["batch_size"],
        "max_epochs": active_config["max_epochs"],
        "learning_rate": active_config["learning_rate"],
        "optimizer": "AdamW (bnb)",
        "quantization": "4-bit",
        "lora_r": active_config["lora_rank"],
        "lora_alpha": active_config["lora_alpha"],
        "lora_dropout": active_config["lora_dropout"],
        "focal_loss_gamma": active_config["focal_loss_gamma"],
        "focal_loss_alpha": active_config["focal_loss_alpha"],

        "eval_mode_for_stopping_scheduling": eval_mode,
        "lr_scheduler_mode": active_config["lr_scheduler_mode"],
        "lr_scheduler_patience": active_config["lr_scheduler_patience"],
        "lr_scheduler_factor": active_config["lr_scheduler_factor"],
        "early_stopping_patience": active_config["early_stopping_patience"],
        "improvement_delta": active_config["improvement_delta"],

        "min_lr": active_config["min_lr"],
        "max_lr_reductions": active_config["max_lr_reductions"],
        "master_seed": active_config["master_seed"],
        "beta1": active_config["beta1"],
        "beta2": active_config["beta2"],
        "learning_rate_phases": active_config["learning_rate_phases"], # Log phase LRs
        "weight_decay_phases": active_config["weight_decay_phases"],   # Log phase WDs
        "grad_clip_norm_phases": active_config["grad_clip_norm_phases"], # Log phase GCNs
        "phase_boundaries": active_config["phase_boundaries"],          # Log conceptual phase boundaries
        "warmup_steps_phases": active_config["warmup_steps_phases"], # Log warmup steps

        # Log new scheduler hyperparameters
        "lr_scheduler_types_phases": active_config["lr_scheduler_types_phases"],
        "cosine_warm_restarts_t_0_phases": active_config["cosine_warm_restarts_t_0_phases"],
        "cosine_warm_restarts_t_mult_phases": active_config["cosine_warm_restarts_t_mult_phases"],
        "step_lr_step_size_phases": active_config["step_lr_step_size_phases"],
        "step_lr_gamma_phases": active_config["step_lr_gamma_phases"],

        # Log new phase-dependent general hyperparameters
        "eval_every_n_steps_phases": active_config["eval_every_n_steps_phases"],

        # Log new early stopping hyperparameters
        "early_stopping_enabled_globally": active_config.get("early_stopping_enabled_globally", False),
        "early_stopping_active_ranges": active_config.get("early_stopping_active_ranges", []),
        "obstacle_category_weights": active_config.get("obstacle_category_weights", {}),
        # "real_image_eval_weight": active_config["real_image_eval_weight"],
        # "sim_image_eval_weight": active_config["sim_image_eval_weight"],
    }
    )
...
last_known_metrics = {
    "eval_loss": 0.0, "overall_eval_acc": 0.0,
    "test_loss": 0.0, "overall_test_acc": 0.0,
}
...
for epoch in range(max_epochs): # epochs loop
    ...
    for batch_idx, batch in enumerate(train_dataloader): # batches loop
        ...
        if current_segment_name != applied_segment_details["name"]:
            ...
            new_wd = active_config["weight_decay_phases"][current_main_phase_idx]
            new_gcn = active_config["grad_clip_norm_phases"][current_main_phase_idx]
            # new_keyword_weight = active_config["keyword_weight_phases"][current_main_phase_idx]
            # new_background_weight = active_config["background_weight_phases"][current_main_phase_idx]
            new_eval_every_n_steps = active_config["eval_every_n_steps_phases"][current_main_phase_idx]

            optimizer.param_groups[0]['weight_decay'] = new_wd
            active_config["grad_clip_norm"] = new_gcn
            # active_config["keyword_weight"] = new_keyword_weight
            # active_config["background_weight"] = new_background_weight
            active_config["eval_every_n_steps"] = new_eval_every_n_steps # Eval only happens in main phase
            ...
            if current_segment_is_warmup:
                ...
                # print(f"  Segment Type: Warmup. LR will ramp from {active_config['min_lr']} to {active_config['learning_rate_phases'][current_main_phase_idx]}.")
                print(f"  Segment Type: Warmup. LR will ramp from {active_config['min_lr']} to {active_config['learning_rate_phases'][current_main_phase_idx]}.")
                print(f"  WD: {new_wd}, Grad Clip: {new_gcn}")
                log_payload["segment_scheduler_type"] = current_scheduler_type
            else: # Main Phase
                new_lr = active_config["learning_rate_phases"][current_main_phase_idx]
                new_scheduler_type_for_main_phase = active_config["lr_scheduler_types_phases"][current_main_phase_idx]
                ...
                if new_scheduler_type_for_main_phase != current_scheduler_type or scheduler is None:
                    ...
                    if new_scheduler_type_for_main_phase == "ReduceLROnPlateau":
                        scheduler = ReduceLROnPlateau(
                            optimizer,
                            mode=active_config["lr_scheduler_mode"],
                            factor=active_config["lr_scheduler_factor"],
                            patience=active_config["lr_scheduler_patience"],
                            verbose=True,
                            min_lr=active_config["min_lr"]
                        )
                        lr_reduction_monitor = LRReductionMonitor()
                    elif new_scheduler_type_for_main_phase == "CosineAnnealingWarmRestarts":
                        t_0_current_phase = active_config["cosine_warm_restarts_t_0_phases"][current_main_phase_idx]
                        t_mult_current_phase = active_config["cosine_warm_restarts_t_mult_phases"][current_main_phase_idx]
                        scheduler = CosineAnnealingWarmRestarts(
                            optimizer,
                            T_0=t_0_current_phase,
                            T_mult=t_mult_current_phase,
                            eta_min=active_config["min_lr"],
                            verbose=True,
                            last_epoch=-1 # Important for re-initialization
                        )
                    elif new_scheduler_type_for_main_phase == "StepLR":
                        step_size_current_phase = active_config["step_lr_step_size_phases"][current_main_phase_idx]
                        gamma_current_phase = active_config["step_lr_gamma_phases"][current_main_phase_idx]
                        scheduler = StepLR(
                            optimizer,
        ...
        if applied_segment_details["is_warmup"]:
            ...
            total_warmup_steps_for_segment = active_config["warmup_steps_phases"][applied_segment_details["main_phase_idx"]]
            target_lr_main_phase = active_config["learning_rate_phases"][applied_segment_details["main_phase_idx"]]
            min_lr_val = active_config["min_lr"]
        ...
        outputs, current_batch_size, labels = custom_forward(batch, model)
        logits  = outputs.logits 
        # loss = weighted_loss_calculation(logits, labels, sentinel_ids, active_config["keyword_weight"], active_config["background_weight"])
        shift_logits = logits[:, :-1, :].contiguous()
        shift_labels = labels[:, 1:].contiguous()
        loss_fct = FocalLoss(
            gamma=active_config["focal_loss_gamma"],
        ...
        # Gradient Clipping
        if active_config["grad_clip_norm"] > 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), active_config["grad_clip_norm"])
        ...
        if not applied_segment_details["is_warmup"] and \
           steps_in_current_phase_for_eval_counter % active_config["eval_every_n_steps"] == 0:
            
            #get eval loss and accuracy
            eval_loss, overall_eval_acc = evaluate_model(model, eval_dataloader, active_config["obstacle_category_weights"])
            print(f"Evaluation - Step {global_step}, Eval Loss: {eval_loss:.4f}, Eval Overall Accuracy: {overall_eval_acc:.4f}")

            #get test loss and accuracy
            test_loss, overall_test_acc = evaluate_model(model, test_dataloader, active_config["obstacle_category_weights"])
            print(f"Test - Step {global_step}, Test Loss: {test_loss:.4f}, Test Overall Accuracy: {overall_test_acc:.4f}")

            current_eval_count +=1

            # Save manual eval checkpoint
            if (checkpoint_counter < len(active_config["manual_checkpoint_steps"])) and (global_step >= active_config["manual_checkpoint_steps"][checkpoint_counter]):
                checkpoint_counter += 1
                manual_eval_chkpoint = os.path.join(active_config["checkpoint_dir"], f"manual_eval_chkpoint_{current_eval_count}/")
                ...
                metadata = {
                    ...
                    "step": global_step,
                    "eval_metric": last_known_metrics["eval_loss"] if eval_mode == "loss" else last_known_metrics["overall_eval_acc"],
                    "test_metric": last_known_metrics["test_loss"] if eval_mode == "loss" else last_known_metrics["overall_test_acc"],
                    "saved_at": time.strftime("%Y-%m-%d %H:%M:%S")
                }
            ...
            if active_config.get("early_stopping_enabled_globally", False):
                active_ranges = active_config.get("early_stopping_active_ranges", [])
            ...
            if is_early_stop_active_now:
                if early_stopper is None: 
                    early_stopper = EarlyStopping(patience=active_config["early_stopping_patience"])
            ...
            if eval_mode == "loss":
                ...
                print(f"Using Eval Loss ({eval_loss:.4f}) for LR scheduling and early stopping.")
            elif eval_mode == "accuracy":
                evaluation_metric = overall_eval_acc
                print(f"Using Eval Accuracy ({overall_eval_acc:.4f}) for LR scheduling and early stopping.")
            else:
            ...
            metric_improved_and_checkpoint_saved = save_checkpoint_if_improved(
                    ...
                    mode=eval_mode,
                    delta=active_config["improvement_delta"]
                ) 
            ...
            if is_early_stop_active_now and early_stopper is not None:
                ...
                if early_stopper.patience_met:
                    if current_scheduler_type == "ReduceLROnPlateau" and scheduler is not None:
                        if lr_reduction_monitor.lr_reduction_count >= active_config["max_lr_reductions"]:
                            print(f"Early stopping triggered: LR reduced {lr_reduction_monitor.lr_reduction_count}/{active_config['max_lr_reductions']} times and {eval_mode} metric ({evaluation_metric:.4f}) hasn't improved for {early_stopper.patience} evaluations. Stopping.")
                            stop_training = True
                        else:
                            print(f"{eval_mode.capitalize()} metric ({evaluation_metric:.4f}) hasn't improved for {early_stopper.patience} evaluations, but max LR reductions ({lr_reduction_monitor.lr_reduction_count}/{active_config['max_lr_reductions']}) not reached. Continuing.")
                    else: 
            ...
            last_known_metrics.update({
                "eval_loss": eval_loss, "overall_eval_acc": overall_eval_acc,
                "test_loss": test_loss, "overall_test_acc": overall_test_acc,
            })
    ...
    if epoch == 0:
        first_epoch_chkpoint = os.path.join(active_config["checkpoint_dir"], "first_epoch_chkpoint/")
...
if not active_config.get("early_stopping_enabled_globally", False):
    ...
    final_checkpoint_path = os.path.join(active_config["checkpoint_dir"], "final_save/")
    ...
    wandb.log({#modify as needed
            ...
            "final_eval_metric": last_known_metrics["eval_loss"] if eval_mode == "loss" else last_known_metrics["overall_eval_acc"],
            "final_test_metric": last_known_metrics["test_loss"] if eval_mode == "loss" else last_known_metrics["overall_test_acc"],
        })
