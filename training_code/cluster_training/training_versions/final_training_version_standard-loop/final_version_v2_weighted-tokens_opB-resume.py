...
hyperparameters = {
    ...
    "run_name_prefix": "1.3b_Option-B/second_version/7th-run_b6_s23_full-epoch", # Prefix for run names and checkpoint subdirs
    ...
    "early_stopping_active_ranges": [[2600, 20000]], # New: List of [start_step, end_step] ranges for active early stopping. Example: [[0, 2000], [4000, 6000]]
    ...
    "early_stopping_patience": 8 if eval_mode == "accuracy" else 1,
    "manual_eval_save_count": [16, 17],
    ...
    "keyword_weight_phases": [1.25, 1.5, 1.5],       # Keyword token weight for each phase (applies to WU and Main)
...
# Set RESUME_TRAINING to True to load an adapter from a previous run and continue training.
# If False, the script will train a new adapter from scratch.
RESUME_TRAINING = True

# If RESUME_TRAINING is True, specify the path to the adapter checkpoint directory
# and the global step at which that training run was saved.
if RESUME_TRAINING:
    # IMPORTANT: Update this path to point to your saved adapter checkpoint directory.
    # For example: "./1.3b_Option-B/second_version/7th-run_b6_s23_full-epoch_loss/best_chkpoint"
    RESUME_ADAPTER_PATH = "/path/to/your/adapter/checkpoint" 
    
    # IMPORTANT: Update this to the global_step value from your previous run's checkpoint.
    # You can often find this in a 'checkpoint_metadata.json' file in the checkpoint directory.
...
def custom_forward(batch, model):
    ...
    assistant_token_ids = tokenizer.encode(assistant_phrase, add_special_tokens=False) # tokenize and get token ids of "Assistant:" as list
    assistant_token_ids_tensor = torch.tensor(assistant_token_ids, device=model.device, dtype=torch.long) #create torch tensor of tokens ids in assistant_token_ids
    
    L_assistant = len(assistant_token_ids) # length of assistant_token_ids list
    current_input_ids = batch["input_ids"] # token ids of input tokens from batch, is a tensor of size [batch_size, padded_sample_length] already on model.device
...
def weighted_loss_calculation(logits, labels, sentinel_ids, keyword_weight, background_weight, slot_table_ref):
    # sentinels_ids is ids of sentinel tokens, slot_table_ref is the slot_table we defined, looks like:
    """slot_table = {
    "<LANE>": encode_list_of_full_phrases(lane_labels_full, tokenizer),
    "<OBS>":  encode_list_of_full_phrases(obs_labels_full, tokenizer),
    "<DEC>":  encode_list_of_full_phrases(dec_labels_full, tokenizer),
    }"""

    """
    Calculates weighted cross-entropy loss.
    Weights are applied to entire key phrases (sentinel + text) found in labels.
    """
    ...
    # sentinel_ids is still useful for getting the sentinel_str keys for slot_table_ref
    for sentinel_str, _ in sentinel_ids.items():
        candidate_full_phrases = slot_table_ref.get(sentinel_str) # Get list: [tensor(id_1, id_2, ...), tensor(), ...] of tokenized key-phrases
        
        if not candidate_full_phrases:
            # This might happen if a sentinel_id exists but has no entries in slot_table, though unlikely with current setup.
            # print(f"Warning (loss): No candidate full phrases found for sentinel '{sentinel_str}' in slot_table_ref.")
            continue

        for batch_idx in range(labels.shape[0]): # For each sample in the batch
            for full_phrase_tensor in candidate_full_phrases: # e.g., tensor for "<LANE> left lane"
                phrase_len = full_phrase_tensor.numel() # length of tensor
                if phrase_len == 0:
                    continue

                # Search for this full_phrase_tensor in the current label sequence
                for seq_idx_start in range(labels.size(1) - phrase_len + 1):
                    label_segment = labels[batch_idx, seq_idx_start : seq_idx_start + phrase_len] # create a segment of the label to be compared with the full phrase
                    
                    if torch.equal(label_segment, full_phrase_tensor.to(labels.device)): # test if the segment is equal to the full phrase (since both are tensors)
                        # Match found! Apply keyword_weight to the entire matched segment.
                        weights[batch_idx, seq_idx_start : seq_idx_start + phrase_len] = keyword_weight
                        # Continue searching in case the same phrase appears multiple times or other phrases for this sentinel type appear.
                        # If phrases for the same sentinel type could overlap and you only want one to apply,
                        # more complex logic would be needed (e.g., longest match or pre-defined priority).
                        # For now, this will weight all occurrences of all defined full phrases.
    ...
    loss_per_tok  = F.cross_entropy(
        ...
        shift_labels.view(-1),
        reduction='none',
        ignore_index=-100  # Make sure padding tokens in labels don't contribute to loss here
    ...
    denominator = (shift_weights * valid_mask).sum() + 1e-10 # Add epsilon to avoid division by zero
...
def compute_sentinel_accuracy(logits: torch.Tensor, labels: torch.Tensor, sentinel_ids: dict, slot_table_ref: dict):
    """
    Computes token-level accuracy for defined full key phrases (sentinel + text).
    Accuracy is calculated for the predictable part of the phrase (i.e., excluding the first token).
    """
    with torch.no_grad():
        shift_logits = logits[:, :-1, :]  # [B, T_max-1, V]
        preds = shift_logits.argmax(-1)   # [B, T_max-1], predictions for labels[:, 1:]

        correct_key_phrase_tokens = {tok_str: 0 for tok_str in sentinel_ids.keys()}
        total_key_phrase_tokens = {tok_str: 0 for tok_str in sentinel_ids.keys()}

        for sentinel_str, _ in sentinel_ids.items(): # Iterate by sentinel type (e.g., "<LANE>")
            candidate_full_phrases_for_sentinel_type = slot_table_ref.get(sentinel_str) #Get list: [tensor(id_1, id_2, ...), tensor(), ...] of tokenized key-phrases
            
            if not candidate_full_phrases_for_sentinel_type:
                # print(f"Warning (accuracy): No candidate full phrases found for sentinel '{sentinel_str}' in slot_table_ref.")
                continue
            
            for batch_idx in range(labels.shape[0]): # For each sample in the batch
                for full_gt_phrase_tensor in candidate_full_phrases_for_sentinel_type: # e.g., tensor for "<LANE> left lane"
                    L_full_phrase = full_gt_phrase_tensor.numel() # length of the full phrase

                    if L_full_phrase <= 1: # Cannot evaluate accuracy if phrase is 0 or 1 token long (nothing to predict)
                        continue
                    
                    num_tokens_to_evaluate_for_acc = L_full_phrase - 1 # We predict L-1 tokens

                    # Search for this full_gt_phrase_tensor in labels[batch_idx]
                    for seq_idx_start_full_phrase in range(labels.size(1) - L_full_phrase + 1):
                        label_segment_full = labels[batch_idx, seq_idx_start_full_phrase : seq_idx_start_full_phrase + L_full_phrase]

                        if torch.equal(label_segment_full, full_gt_phrase_tensor.to(labels.device)):
                            # Found a ground truth full phrase occurrence.
                            
                            # The ground truth tokens we are trying to predict are the phrase excluding its first token.
                            # labels[batch_idx, (seq_idx_start_full_phrase + 1) : (seq_idx_start_full_phrase + L_full_phrase)]
                            
                            # The predictions for these tokens are in:
                            # preds[batch_idx, seq_idx_start_full_phrase : (seq_idx_start_full_phrase + L_full_phrase - 1)]

                            # Check bounds for preds (which is T-1 in sequence length)
                            if seq_idx_start_full_phrase + L_full_phrase - 1 <= preds.size(1):
                                actual_tokens_in_gt_to_predict = labels[batch_idx, 
                                                                        seq_idx_start_full_phrase + 1 : 
                                                                        seq_idx_start_full_phrase + L_full_phrase]
                                
                                predicted_tokens_segment = preds[batch_idx, 
                                                                 seq_idx_start_full_phrase : 
                                                                 seq_idx_start_full_phrase + L_full_phrase - 1]
                                
                                correct_count_this_instance = (predicted_tokens_segment == actual_tokens_in_gt_to_predict).sum().item()
                                
                                correct_key_phrase_tokens[sentinel_str] += correct_count_this_instance
                                total_key_phrase_tokens[sentinel_str] += num_tokens_to_evaluate_for_acc
                            # else:
                                # This full phrase is too close to the end for its predictable part to be fully covered by `preds`.
                                # print(f"Warning (accuracy): GT phrase for {sentinel_str} at batch {batch_idx}, seq {seq_idx_start_full_phrase} (len {L_full_phrase}) extends beyond prediction range.")
        return correct_key_phrase_tokens, total_key_phrase_tokens
...
def evaluate_model(model, data_dataloader):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_dataloader):
            ...
            loss = weighted_loss_calculation(logits, labels, sentinel_ids, hyperparameters["keyword_weight"], hyperparameters["background_weight"], slot_table) # MODIFIED: Added slot_table
            ...
            correct_count, total_count = compute_sentinel_accuracy(logits, labels, sentinel_ids, slot_table) # MODIFIED: Added slot_table

            for sent_tok_str in sentinel_ids.keys(): # Iterate by string key
                global_correct[sent_tok_str] += correct_count[sent_tok_str]
                global_total[sent_tok_str] += total_count[sent_tok_str]
    ...
    sent_acc = {sent_tok_str: (global_correct[sent_tok_str] / global_total[sent_tok_str] if global_total[sent_tok_str] > 0 else 0.0) # Ensure no division by zero
                for sent_tok_str in sentinel_ids.keys()} # Iterate by string key
    
    sum_global_correct_values = sum(global_correct.values())
    sum_global_total_values = sum(global_total.values())

    overall_acc = (sum_global_correct_values / sum_global_total_values) if sum_global_total_values > 0 else 0.0
...
# ---- Define Slot Table for Key Phrases (Option B) ----
# Each phrase now explicitly includes its sentinel token.
lane_labels_full = ["<LANE> left lane", "<LANE> right lane", "<LANE> unclear"]
obs_labels_full  = ["<OBS> not on the same lane", "<OBS> far away", "<OBS> near", "<OBS> very close"]
dec_labels_full  = ["<DEC> straight forward", "<DEC> slow cruise", "<DEC> switch lane"]

def encode_list_of_full_phrases(lst_full_phrases, tokenizer_ref):
    """Encodes a list of (already complete) full phrase strings into a list of token ID tensors."""
    return [torch.tensor(tokenizer_ref.encode(phrase_str, add_special_tokens=False), dtype=torch.long)
            for phrase_str in lst_full_phrases]
...
print("Slot table initialized with explicitly defined full key phrases (sentinel + text) for weighting and accuracy.")
# ---- End Slot Table Definition ----
...
if RESUME_TRAINING:
    print(f"Resuming training. Loading adapter from: {RESUME_ADAPTER_PATH}")
    # Load the base model with the saved adapter weights.
    model = PeftModel.from_pretrained(model, RESUME_ADAPTER_PATH, adapter_name="adapter", is_trainable=True)
    print("Adapter loaded successfully.")
else:
    print("Starting new training. Initializing new LoRA adapter.")
    # Apply new LoRA adapters to the base model.
...
# --- Training Loop Configuration for Resuming ---
# These will be calculated if resuming, otherwise they remain 0.
start_epoch = 0
start_batch_idx = 0
if RESUME_TRAINING:
    # Ensure train_dataloader has been created and is not empty
    if 'train_dataloader' in locals() and len(train_dataloader) > 0:
        steps_per_epoch = len(train_dataloader)
        start_epoch = RESUME_FROM_STEP // steps_per_epoch
        start_batch_idx = RESUME_FROM_STEP % steps_per_epoch
        print(f"Resuming from Epoch {start_epoch + 1}, Batch {start_batch_idx + 1} (based on {steps_per_epoch} steps per epoch).")
    else:
        # This warning might appear if the script is run in a context where train_dataloader isn't immediately available,
        # or if the training dataset is empty.
        print("Warning: Could not determine steps_per_epoch. Resuming will start from the beginning of epoch 0, which may lead to processing duplicate data if RESUME_FROM_STEP > 0.")
...
for epoch in range(start_epoch, max_epochs): # epochs loop
    ...
    for batch_idx, batch in enumerate(train_dataloader): # batches loop
        ...
        # --- Skip batches if resuming training from a specific point ---
        # This ensures the training data stream aligns with the resumed global_step.
        # This check is only active for the first epoch of the resumed run.
        if RESUME_TRAINING and epoch == start_epoch and batch_idx < start_batch_idx:
            continue
                ...
                if new_scheduler_type_for_main_phase != current_scheduler_type or scheduler is None:
                    ...
                    for group in optimizer.param_groups:
                        if 'initial_lr' in group:
                            del group['initial_lr'] # delete old learning rate
