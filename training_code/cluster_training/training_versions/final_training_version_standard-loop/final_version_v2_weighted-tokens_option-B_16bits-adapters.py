...
hyperparameters = {
    ...
    "eval_every_n_steps_phases": [100, 50, 50],   # Evaluation frequency for each main phase (evaluation skipped in WU)
...
# Cast trainable LoRA adapter parameters (and any specified in modules_to_save) to a lower precision
# It's good practice to use bfloat16 if supported, as recommended by the QLoRA paper.
# Otherwise, float16 can be used.
desired_adapter_dtype = torch.bfloat16
if desired_adapter_dtype == torch.bfloat16 and hasattr(torch.cuda, 'is_bf16_supported') and not torch.cuda.is_bf16_supported():
    print("Warning: BF16 is selected for LoRA adapters, but not supported by the current GPU. Falling back to FP16.")
    desired_adapter_dtype = torch.float16
elif desired_adapter_dtype == torch.bfloat16 and not hasattr(torch.cuda, 'is_bf16_supported'):
    # Fallback for older PyTorch versions or CPU execution if is_bf16_supported is not available
    # A more robust check might be needed depending on the environment
    try:
        # Try a small bf16 operation to check support
        _ = torch.tensor([1.0]).to(desired_adapter_dtype)
    except RuntimeError:
        print("Warning: BF16 is selected for LoRA adapters, but seems unsupported. Falling back to FP16.")
        desired_adapter_dtype = torch.float16


print(f"Casting trainable parameters (LoRA adapters and modules_to_save) to {desired_adapter_dtype}...")
for name, param in model.named_parameters():
    if param.requires_grad and "lora" in name: # This now targets only LoRA adapter weights
        # print(f"  Casting {name} from {param.dtype} to {desired_adapter_dtype}") # Optional: for verbose logging
        param.data = param.data.to(desired_adapter_dtype)

# You can print trainable parameters to verify their dtypes and count
model.print_trainable_parameters()
# Example to verify dtypes after casting:
# for name, param in model.named_parameters():
#     if param.requires_grad:
#         print(f"  Trainable param: {name}, Dtype: {param.dtype}, Device: {param.device}")
        ...
        if not applied_segment_details["is_warmup"] and \
           steps_in_current_phase_for_eval_counter % hyperparameters["eval_every_n_steps"] == 0:
            ...
            if current_eval_count in hyperparameters["manual_eval_save_count"] and hyperparameters["manual_eval_save"]:
                manual_eval_chkpoint = os.path.join(hyperparameters["checkpoint_dir"], f"manual_eval_chkpoint{current_eval_count}/")
