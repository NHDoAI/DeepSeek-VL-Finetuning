...
hyperparameters = {
    ...
    "run_name_prefix": "1.3b_shortprompt/focalloss-test/080725_0400_pre-trim_b6-s42_short-lidar_focal-loss_custom-alpha", # Prefix for run names and checkpoint subdirs
    ...
    "eval_every_n_steps_phases": [100, 100, 50],   # Evaluation frequency for each main phase (evaluation skipped in WU)
     ...
     # obstacle category weights for weighted evaluation loss calculation. In this case, all categories are weighted equally, effectively the same as not using weights. The reason is in theory with the custom alpha values, the loss is already weighted.
...
class FocalLoss(torch.nn.Module):
    ...
    def forward(self, input, target):
        ...
        full_focal_loss[mask] = focal_loss_values.to(full_focal_loss.dtype)
