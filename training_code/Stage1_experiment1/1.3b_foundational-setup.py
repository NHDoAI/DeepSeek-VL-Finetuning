...
hyperparameters = {
    ...
    "run_name_prefix": "1.3b/foundational-setup_s42", # Prefix for run names and checkpoint subdirs
    ...
    "early_stopping_patience": 5 if eval_mode == "accuracy" else 3, # how many eval steps in which there is no improvement before early stopping is considered. This is the second condition for early stopping.
...
def evaluate_model(model, data_dataloader):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_dataloader):
            ...
            loss_fct = torch.nn.CrossEntropyLoss(ignore_index=-100) # initialize the loss function with the ignore_index
            loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1)) # computes the average loss for all valid (non "-100") tokens in the batch.
            ...
            valid_mask_acc = (shift_labels != -100) # creates a boolean mask that is True for all tokens we should care about and False for all the ignored tokens (padding/prompt).
            num_correct_tokens += (preds_acc[valid_mask_acc] == shift_labels[valid_mask_acc]).sum().item() # count the number of correct tokens
            num_total_tokens += valid_mask_acc.sum().item() # counts the total number of valid tokens in the batch and adds it to the running total.
    ...
    avg_loss = total_loss / total_samples if total_samples > 0 else 0 # calculate the average loss for all batches

    overall_acc = num_correct_tokens / num_total_tokens if num_total_tokens > 0 else 0.0 # calculate the overall accuracy
    ...
    for batch_idx, batch in enumerate(train_dataloader): # batches loop
        ...
        if not applied_segment_details["is_warmup"] and \
           steps_in_current_phase_for_eval_counter % hyperparameters["eval_every_n_steps"] == 0:
            ...
            eval_loss_real, overall_eval_acc_real = evaluate_model(model, eval_dataloader_real)
            ...
            eval_loss_sim, overall_eval_acc_sim = evaluate_model(model, eval_dataloader_sim)
            ...
            wandb.log({
                ...
                "sent_eval_acc_real": last_sent_eval_acc_real,
                "sent_eval_acc_sim": last_sent_eval_acc_sim,
            ...
            last_eval_loss_real, last_overall_eval_acc_real, last_eval_loss_sim, last_overall_eval_acc_sim, last_avg_eval_loss, last_avg_eval_acc = eval_loss_real, overall_eval_acc_real, eval_loss_sim, overall_eval_acc_sim, avg_eval_loss, avg_eval_acc
