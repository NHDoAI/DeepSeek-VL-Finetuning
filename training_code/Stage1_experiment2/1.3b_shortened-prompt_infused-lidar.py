...
hyperparameters = {
    ...
    "run_name_prefix": "1.3b_shortprompt/07072025_0230_pre-trim_b6-s42_single-lidar", # Prefix for run names and checkpoint subdirs
    ...
    # obstacle category weights for weighted evaluation loss calculation, to reduce the impact of categories imbalance
...
def evaluate_model(model, data_dataloader, obstacle_weights):
    """
    Evaluate the model on the evaluation dataset, with loss and accuracy weighted by obstacle category, defined by hyperparameters["obstacle_category_weights"].
...
train_labels_dir = os.getenv('TRAIN_LABELS_PATH_SINGLELIDAR') # get the path to the training labels directory (in this case with user prompt containing the single minimum lidar reading) from the environment variables
print(train_labels_dir)

eval_labels_dir_real = os.getenv('EVAL_LABELS_PATH_REAL_SINGLELIDAR') # get the path to the evaluation labels real samples directory from the environment variables
print(f"Eval Real Labels Path: {eval_labels_dir_real}")
eval_labels_dir_sim = os.getenv('EVAL_LABELS_PATH_SIM_SINGLELIDAR')
...
dataset_artifact.add_dir("./train/labels_singlelidar")
dataset_artifact.add_dir("./eval/real/images")
dataset_artifact.add_dir("./eval/real/labels_singlelidar")
